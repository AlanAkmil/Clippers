import type { AiHighlightSuggestion, Highlight, HighlightScore, SubtitleCue } from "./types";
...
export interface ScoreInput {
  ...
  clipLength: number;
  clipCount: number;
  /** Viral-moment windows suggested by the AI scoring model, if AI assist was used. */
  aiSuggestions?: AiHighlightSuggestion[];
}

/** Windowed highlight scoring with diversity-aware selection, optionally boosted by AI picks. */
export function detectHighlights({
  ...
  const scoreWindow = (start: number, end: number): { score: number; parts: HighlightScore } => {
    ...
    if (slice.length === 0) {
      return { score: 0, parts: { audio: 0, speech: 0, silence: 0, motion: 0, keywords: 0 } };
    }
  ...
  for (let start = 0; start + length <= Math.max(length, safeDuration); start += step) {
    ...
    const { score, parts } = scoreWindow(start, end);
    if (parts.audio === 0 && parts.speech === 0 && parts.motion === 0 && parts.keywords === 0) continue;
    candidates.push({ id: `${start.toFixed(2)}-${end.toFixed(2)}`, start, end, score, parts, source: "heuristic" });
  }
  candidates.sort((a, b) => b.score - a.score);

  // AI-suggested windows are blended with the heuristic signal for that same window,
  // then given first pick over the pure-heuristic candidates.
  const aiCandidates: Highlight[] = aiSuggestions
    .filter((window) => window.end > window.start && window.start >= 0 && window.start < safeDuration)
    .map((window) => {
      const end = Math.min(window.end, safeDuration);
      const { score: heuristicScore, parts } = scoreWindow(window.start, end);
      const blended = clamp(heuristicScore * 0.35 + (clamp(window.score / 100)) * 0.65);
      return {
        id: `ai-${window.start.toFixed(2)}-${end.toFixed(2)}`,
        start: window.start,
        end,
        score: blended,
        parts,
