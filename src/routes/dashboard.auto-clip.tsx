...
function AutoClipPage() {
  ...
  const runAiAssist = useCallback(async () => {
    if (!file) {
      toast.error("Upload video dulu sebelum pakai AI assist.");
      return;
    }
    setAiBusy(true);
    try {
      ...
      useAutoClipStore.getState().setCues(aiCues, `AI transcript${language ? ` (${language})` : ""}`);
      toast.success(`${aiCues.length} baris transkrip dari AI`);

      setAiStatus("AI lagi nyari momen paling viral…");
      const suggestions = await generateAiHighlights(
        aiCues,
        source?.duration ?? 0,
        config.clipLength,
        config.clipCount,
      ...
      <div className="mt-8 grid min-w-0 gap-4 lg:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)]">
        <div className="flex min-w-0 flex-col gap-4">
          ...
          <StepCard
            step={6}
            title="Subtitles & AI"
            description="Import SRT/VTT manual, atau biarin AI bikin transkrip + milih momen viral."
            complete
          >
            <div className="flex flex-col gap-3 rounded-2xl bg-secondary/40 p-3">
              <div className="flex items-center justify-between gap-4">
                <div className="min-w-0">
                  <p className="text-sm font-medium">Generate pakai AI (Groq)</p>
                  <p className="mt-0.5 text-xs text-muted-foreground">
                    Auto-transkrip audio + AI milih rentang waktu paling berpotensi viral.
                  </p>
                </div>
                <Button
                  size="sm"
